...
arrange(Monitor *m)
{
	...
	if (m->lt[m->sellt]->arrange) {
		...
		wl_list_for_each(c, &clients, link) {
			...
			if (enablegaps)
				applygaps(c);
			resizeapply(c, c->geom, /*c->interact*/0);
		}
...
setup(void)
{
	...
	idle_notifier = /*wlr_idle_notifier_v1_create(dpy)*/NULL;
	...
	setenv("XCURSOR_SIZE", cursorSizeStr, 1);
...
{	
	...
	wl_list_for_each(kb, &keyboards, link) {
		...
		uint32_t leds = 0;
		xkb_mod_mask_t locked_mods = 0;
		// Note: forces numlock and capslock even if they were changed
		if (numlock) {
			xkb_mod_index_t mod_index = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_NUM);
			if (mod_index != XKB_MOD_INVALID) {
				locked_mods |= (uint32_t)1 << mod_index;
		...
		if (locked_mods) {
			wlr_keyboard_notify_modifiers(kb->wlr_keyboard, 0, 0,
				locked_mods, 0);

			for (uint32_t i = 0; i < WLR_LED_COUNT; ++i) {
				if (xkb_state_led_index_is_active(
						kb->wlr_keyboard->xkb_state,
						kb->wlr_keyboard->led_indexes[i])) {
					leds |= (1 << i);
				}
			}
			wlr_keyboard_led_update(kb->wlr_keyboard, leds);
		}
		xkb_keymap_unref(keymap);
		xkb_context_unref(context);
	}
